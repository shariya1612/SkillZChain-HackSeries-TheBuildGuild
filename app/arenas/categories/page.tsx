...
import { categories, soloGames, multiplayerGames } from "@/data/games"
...
export default function CategoriesPage() {
  ...
  const categoryRefs = useRef<Record<string, HTMLDivElement | null>>(
    categories.reduce((acc, category) => {
      acc[category.name.toLowerCase()] = null
      return acc
    }, {}),
  )

  // Handle scrolling to category on mount
  useEffect(() => {
    if (categoryParam) {
      const categoryRef = categoryRefs.current[categoryParam.toLowerCase()]
      if (categoryRef) {
        setTimeout(() => {
  ...
  // Get all games (both solo and multiplayer)
  ...
  const gamesByCategory = categories.reduce(
    (acc, category) => {
      acc[category.name] = filteredGames.filter((game) => game.category.toLowerCase() === category.name.toLowerCase())
      return acc
    },
          ...
          <div className="container mx-auto px-4 py-6 md:py-8">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6 md:mb-8">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-2xl md:text-3xl font-bold text-white">Game Categories</h1>
              </div>
              ...
              <div className="w-full sm:w-auto sm:ml-auto relative">
                ...
                <input
                  type="text"
                  placeholder="Search games across categories..."
                  value={searchTerm}
            ...
            {categories.map((category) => {
              const games = gamesByCategory[category.name] || []

              // Skip empty categories when searching
              if (searchTerm && games.length === 0) return null
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 mb-4 md:mb-6">
                    <div className="bg-slate-800 p-2 md:p-3 rounded-full">
                      <div className="h-5 w-5 md:h-6 md:w-6 text-blue-400"></div>
                    </div>
                    <h2 className="text-xl md:text-2xl font-bold">{category.name} Games</h2>
                  </div>
                  ...
                  {games.length > 0 ? (
                    ...
                    <div className="bg-slate-800/30 p-6 md:p-8 text-center rounded-lg border border-slate-700">
                      <p className="text-slate-400 text-sm md:text-base">No {category.name} games found.</p>
                    </div>
                  )}
                </div>
              )
            ...
            {/* If no games match search */}
            {filteredGames.length === 0 && (
              <div className="bg-slate-800/30 p-6 md:p-8 text-center rounded-lg border border-slate-700 mt-8">
                <h3 className="text-xl font-bold mb-4">No Games Found</h3>
                <p className="text-slate-400 text-sm md:text-base mb-4">No games match your search criteria.</p>
                <Button onClick={() => setSearchTerm("")} className="bg-slate-700 hover:bg-slate-600">
                  Clear Search
                </Button>
              </div>
            )}
